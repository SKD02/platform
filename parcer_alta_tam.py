...
def _parse_svh_address(address: str):
    """
    Преобразуем строку адреса ALTA во что-то типа:

      CountryCode   -> RU
      CountryName   -> РОССИЯ
      Region        -> САНКТ-ПЕТЕРБУРГ
      City          -> ПОС.ШУШАРЫ
      StreetHouse   -> УЧАСТОК Ж.Д."МОСКОВСКОЕ ШОССЕ-РЕКА КУЗЬМИНКА"

    Эвристика под форматы:
      "RU - 196626, Санкт-Петербург, п. Шушары, уч. ж/д \"...\""
      "RU - 196626, Санкт-Петербург, Московское ш., д. 54А"
    """
    if not address:
        return {
            "CountryCode": "",
