...
def credits_spend(
    *, user_id: int, amount: int,
    ...
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT credits_remaining, period_end FROM user_credits_balance WHERE user_id=%s FOR UPDATE",
            (user_id,),
        )
        row = cur.fetchone()
        if not row:
            raise ValueError("Нет активного пакета. Перейдите к оплате.")

        credits_remaining = int(row["credits_remaining"] or 0)
        period_end = row["period_end"]

        cur.execute("SELECT now() as n")
        now_ts = cur.fetchone()["n"]

        if period_end and now_ts > period_end:
            # пакет истёк
            raise ValueError("Срок пакета истёк. Перейдите к оплате и купите новый пакет.")

        if credits_remaining < amount:
            raise ValueError("Недостаточно запросов. Перейдите к оплате и купите пакет.")
        ...
        cur.execute(
            ...
            """,
            (user_id, -amount, reason, ref_type, ref_id, meta_json),
        )
